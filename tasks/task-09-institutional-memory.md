# Task 9 — Institutional Memory

**Answers:** "who's in charge now, and what changed?" — build a history over years.
**Suggested cadence:** weekly

> Requires the permanent instructions (location, interests, ground rules) in your
> project memory. See [`../prompts/PERMANENT_INSTRUCTIONS.md`](../prompts/PERMANENT_INSTRUCTIONS.md).

## Scheduled prompt (paste this as the task)

```text
Maintain the institutional record for my area, across ALL levels that govern me —
local, state/province, and national/federal. Each week, update:
- departments/agencies and what they do (at every level)
- new directors / commissioners / department heads, and state/national office
  holders whose remit reaches me (and who they replaced)
- agencies merged, split, renamed, or given new responsibilities
- significant new legislation or regulation (local ordinance, state law, or
  national act) and which body enacted it
For each change, tag the level, name the office (not just the person), and cite the
source. Over time this becomes a queryable history of who ran what, when, at each level.
```

---

Part of [Citizen Copilot](../README.md). Adapt the prompt to your locality and
assistant; keep the "separate fact from opinion / prefer official sources" rules.
